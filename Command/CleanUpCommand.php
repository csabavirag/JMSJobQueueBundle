...
{
    public function __construct(private readonly Registry $registry, private readonly JobManager $jobManager)
    {
        parent::__construct();
    }
    ...
    private function cleanUpExpiredJobs(EntityManager $em, Connection $con, InputInterface $input): void
    {
        ...
        foreach ($this->findExpiredJobs($em, $input) as $job) {
            ...
            $result = $con->executeQuery($incomingDepsSql, [
                'id' => $job->getId(),
            ]);
    ...
    private function resolveDependencies(EntityManager $em, Job $job): void
    {
        ...
        $em->getConnection()->executeUpdate("DELETE FROM jms_job_dependencies WHERE dest_job_id = :id", [
            'id' => $job->getId(),
        ]);
    }
